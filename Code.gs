...
function updateTrekkerDetails(email, trekDate, index, editedData) {
  try {
    ...
    const genderIdx = headers.findIndex(h => (h || '').toLowerCase() === 'gender');
    const ageIdx = headers.findIndex(h => (h || '').toLowerCase() === 'age');
    const idTypeIdx = headers.findIndex(h => (h || '').toLowerCase() === 'id type');
    const idNumberIdx = headers.findIndex(h => (h || '').toLowerCase() === 'id number' || (h || '').toLowerCase() === 'idnumber');
    const mobileIdx = headers.findIndex(h => (h || '').toLowerCase() === 'mobile number' || (h || '').toLowerCase() === 'mobile');
    const isForeignerIdx = headers.findIndex(h => (h || '').toLowerCase() === 'isforeigner' || (h || '').toLowerCase() === 'is foreigner' || (h || '').toLowerCase() === 'is_foreigner' );
    ...
    if (nameIdx !== -1 && editedData.Name !== undefined) sheet.getRange(rowNumber, nameIdx + 1).setValue(editedData.Name || '');
    if (genderIdx !== -1 && editedData.Gender !== undefined) sheet.getRange(rowNumber, genderIdx + 1).setValue(editedData.Gender || '');
    if (ageIdx !== -1 && editedData.Age !== undefined) {
      const ageNum = Number(editedData.Age);
      if (isNaN(ageNum) || ageNum < 14) return { success: false, message: 'Age must be a number and at least 14 years.' };
      sheet.getRange(rowNumber, ageIdx + 1).setValue(editedData.Age || '');
    }
    if (idTypeIdx !== -1 && editedData['Photo ID Type'] !== undefined) sheet.getRange(rowNumber, idTypeIdx + 1).setValue(editedData['Photo ID Type'] || '');
    if (idNumberIdx !== -1 && editedData['ID Number'] !== undefined) sheet.getRange(rowNumber, idNumberIdx + 1).setValue(editedData['ID Number'] || '');
    if (mobileIdx !== -1 && editedData['Mobile Number'] !== undefined) sheet.getRange(rowNumber, mobileIdx + 1).setValue(editedData['Mobile Number'] || '');

    // Update IsForeigner if provided (expect boolean or 'Yes'/'No')
    if (isForeignerIdx !== -1 && editedData.IsForeigner !== undefined) {
      const val = (editedData.IsForeigner === true || ('' + editedData.IsForeigner).toLowerCase() === 'yes' || ('' + editedData.IsForeigner).toLowerCase() === 'true') ? 'Yes' : 'No';
      sheet.getRange(rowNumber, isForeignerIdx + 1).setValue(val);
    }

    // Update Emergency Contact for the row (group-level info) if provided
    if (emergencyIdx !== -1 && editedData.EmergencyContact !== undefined) {
      sheet.getRange(rowNumber, emergencyIdx + 1).setValue(editedData.EmergencyContact || '');
      // Also update all rows in this group so Emergency Contact is consistent (optional)
      for (let rr of groupRowNums) {
        try { sheet.getRange(rr + 1, emergencyIdx + 1).setValue(editedData.EmergencyContact || ''); } catch(e){}
      }
    }
...
function issueTicketAndEmail(email, date) {
  ...
  // Find header indexes robustly
  const receiptIdx = headers.findIndex(h => (h || '').toString().toLowerCase() === 'paymentreceiptfileid' || (h || '').toString().toLowerCase() === 'payment receipt fileid' || (h || '').toString().toLowerCase() === 'payment receipt file id');
  const amountIdx = headers.findIndex(h => (h || '').toString().toLowerCase() === 'amount remitted' || (h || '').toString().toLowerCase() === 'amount');
  const paymentIdIdx = headers.findIndex(h => (h || '').toString().toLowerCase() === 'payment id' || (h || '').toString().toLowerCase() === 'paymentid');
  const emergencyIdx = headers.findIndex(h => (h || '').toString().toLowerCase() === 'emergency contact' || (h || '').toString().toLowerCase() === 'emergency_contact' || (h || '').toString().toLowerCase() === 'emergency');

  const amountRemitted = firstRow[amountIdx] || '';
  const paymentId = firstRow[paymentIdIdx] || '';
